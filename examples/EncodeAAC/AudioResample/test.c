...
// 读取数据并编码
static void read_data_and_encode(AVFormatContext *fmt_ctx, AVCodecContext *codec_ctx, SwrContext *swr_ctx, FILE *outfile) {
    // 重采样的输入缓冲区
    ...
    while (av_read_frame(fmt_ctx, &sample_pkt) == 0 &&
        ...
        // 开始编码
        encode(codec_ctx, frame, encode_pkt, outfile);
...
void rec_audio() {
    ...
    //打开编码器上下文
    AVCodecContext *c_ctx = create_codec_context();
